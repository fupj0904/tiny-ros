package com.roslib.world_canvas_msgs;

import java.lang.*;

public class PubAnnotationsData {

public static final java.lang.String PUBANNOTATIONSDATA = "world_canvas_msgs/PubAnnotationsData";

public class PubAnnotationsDataRequest implements com.roslib.ros.Msg {
    private long __id__;
    public com.roslib.uuid_msgs.UniqueID[] annotation_ids;
    public java.lang.String topic_name;
    public java.lang.String topic_type;
    public boolean pub_as_list;

    public PubAnnotationsDataRequest() {
        this.annotation_ids = null;
        this.topic_name = "";
        this.topic_type = "";
        this.pub_as_list = false;
        this.__id__ = 0;
    }

    public int serialize(byte[] outbuffer, int start) {
        int offset = start;
        outbuffer[offset + 0] = (byte)((this.__id__ >> (8 * 0)) & 0xFF);
        outbuffer[offset + 1] = (byte)((this.__id__ >> (8 * 1)) & 0xFF);
        outbuffer[offset + 2] = (byte)((this.__id__ >> (8 * 2)) & 0xFF);
        outbuffer[offset + 3] = (byte)((this.__id__ >> (8 * 3)) & 0xFF);
        offset += 4;
        int length_annotation_ids = this.annotation_ids != null ? this.annotation_ids.length : 0;
        outbuffer[offset + 0] = (byte)((length_annotation_ids >> (8 * 0)) & 0xFF);
        outbuffer[offset + 1] = (byte)((length_annotation_ids >> (8 * 1)) & 0xFF);
        outbuffer[offset + 2] = (byte)((length_annotation_ids >> (8 * 2)) & 0xFF);
        outbuffer[offset + 3] = (byte)((length_annotation_ids >> (8 * 3)) & 0xFF);
        offset += 4;
        for (int i = 0; i < length_annotation_ids; i++){
        offset = this.annotation_ids[i].serialize(outbuffer, offset);
        }
        int length_topic_name = this.topic_name.getBytes().length;
        outbuffer[offset + 0] = (byte)((length_topic_name >> (8 * 0)) & 0xFF);
        outbuffer[offset + 1] = (byte)((length_topic_name >> (8 * 1)) & 0xFF);
        outbuffer[offset + 2] = (byte)((length_topic_name >> (8 * 2)) & 0xFF);
        outbuffer[offset + 3] = (byte)((length_topic_name >> (8 * 3)) & 0xFF);
        offset += 4;
        for (int k=0; k<length_topic_name; k++) {
            outbuffer[offset + k] = (byte)((this.topic_name.getBytes())[k] & 0xFF);
        }
        offset += length_topic_name;
        int length_topic_type = this.topic_type.getBytes().length;
        outbuffer[offset + 0] = (byte)((length_topic_type >> (8 * 0)) & 0xFF);
        outbuffer[offset + 1] = (byte)((length_topic_type >> (8 * 1)) & 0xFF);
        outbuffer[offset + 2] = (byte)((length_topic_type >> (8 * 2)) & 0xFF);
        outbuffer[offset + 3] = (byte)((length_topic_type >> (8 * 3)) & 0xFF);
        offset += 4;
        for (int k=0; k<length_topic_type; k++) {
            outbuffer[offset + k] = (byte)((this.topic_type.getBytes())[k] & 0xFF);
        }
        offset += length_topic_type;
        outbuffer[offset] = (byte)((pub_as_list ? 0x01 : 0x00) & 0xFF);
        offset += 1;
        return offset;
    }

    public int deserialize(byte[] inbuffer, int start) {
        int offset = start;
        this.__id__  = (long)((inbuffer[offset + 0] & 0xFF) << (8 * 0));
        this.__id__ |= (long)((inbuffer[offset + 1] & 0xFF) << (8 * 1));
        this.__id__ |= (long)((inbuffer[offset + 2] & 0xFF) << (8 * 2));
        this.__id__ |= (long)((inbuffer[offset + 3] & 0xFF) << (8 * 3));
        offset += 4;
        int length_annotation_ids = (int)((inbuffer[offset + 0] & 0xFF) << (8 * 0));
        length_annotation_ids |= (int)((inbuffer[offset + 1] & 0xFF) << (8 * 1));
        length_annotation_ids |= (int)((inbuffer[offset + 2] & 0xFF) << (8 * 2));
        length_annotation_ids |= (int)((inbuffer[offset + 3] & 0xFF) << (8 * 3));
        offset += 4;
        if(length_annotation_ids > 0) {
            this.annotation_ids = new com.roslib.uuid_msgs.UniqueID[length_annotation_ids];
        }
        for (int i = 0; i < length_annotation_ids; i++){
        offset = this.annotation_ids[i].deserialize(inbuffer, offset);
        }
        int length_topic_name = (int)((inbuffer[offset + 0] & 0xFF) << (8 * 0));
        length_topic_name |= (int)((inbuffer[offset + 1] & 0xFF) << (8 * 1));
        length_topic_name |= (int)((inbuffer[offset + 2] & 0xFF) << (8 * 2));
        length_topic_name |= (int)((inbuffer[offset + 3] & 0xFF) << (8 * 3));
        offset += 4;
        byte[] bytes_topic_name = new byte[length_topic_name];
        for(int k= 0; k< length_topic_name; k++){
            bytes_topic_name[k] = (byte)(inbuffer[k + offset] & 0xFF);
        }
        this.topic_name = new java.lang.String(bytes_topic_name);
        offset += length_topic_name;
        int length_topic_type = (int)((inbuffer[offset + 0] & 0xFF) << (8 * 0));
        length_topic_type |= (int)((inbuffer[offset + 1] & 0xFF) << (8 * 1));
        length_topic_type |= (int)((inbuffer[offset + 2] & 0xFF) << (8 * 2));
        length_topic_type |= (int)((inbuffer[offset + 3] & 0xFF) << (8 * 3));
        offset += 4;
        byte[] bytes_topic_type = new byte[length_topic_type];
        for(int k= 0; k< length_topic_type; k++){
            bytes_topic_type[k] = (byte)(inbuffer[k + offset] & 0xFF);
        }
        this.topic_type = new java.lang.String(bytes_topic_type);
        offset += length_topic_type;
        this.pub_as_list = (boolean)((inbuffer[offset] & 0xFF) != 0 ? true : false);
        offset += 1;
        return offset;
    }

    public int serializedLength() {
        int length = 0;
        length += 4;
        int length_annotation_ids = this.annotation_ids != null ? this.annotation_ids.length : 0;
        for (int i = 0; i < length_annotation_ids; i++) {
        length += this.annotation_ids[i].serializedLength();
        }
        int length_topic_name = this.topic_name.getBytes().length;
        length += 4;
        length += length_topic_name;
        int length_topic_type = this.topic_type.getBytes().length;
        length += 4;
        length += length_topic_type;
        length += 1;
        return length;
    }

    public java.lang.String getType() { return PUBANNOTATIONSDATA; }
    public java.lang.String getMD5(){ return "20b5306d916a1b90c4c4fb95eeb7b121"; }
    public long getID() { return this.__id__; }
    public void setID(long id) { this.__id__ = id; }
}

public class PubAnnotationsDataResponse implements com.roslib.ros.Msg {
    private long __id__;
    public boolean result;
    public java.lang.String message;

    public PubAnnotationsDataResponse() {
        this.result = false;
        this.message = "";
        this.__id__ = 0;
    }

    public int serialize(byte[] outbuffer, int start) {
        int offset = start;
        outbuffer[offset + 0] = (byte)((this.__id__ >> (8 * 0)) & 0xFF);
        outbuffer[offset + 1] = (byte)((this.__id__ >> (8 * 1)) & 0xFF);
        outbuffer[offset + 2] = (byte)((this.__id__ >> (8 * 2)) & 0xFF);
        outbuffer[offset + 3] = (byte)((this.__id__ >> (8 * 3)) & 0xFF);
        offset += 4;
        outbuffer[offset] = (byte)((result ? 0x01 : 0x00) & 0xFF);
        offset += 1;
        int length_message = this.message.getBytes().length;
        outbuffer[offset + 0] = (byte)((length_message >> (8 * 0)) & 0xFF);
        outbuffer[offset + 1] = (byte)((length_message >> (8 * 1)) & 0xFF);
        outbuffer[offset + 2] = (byte)((length_message >> (8 * 2)) & 0xFF);
        outbuffer[offset + 3] = (byte)((length_message >> (8 * 3)) & 0xFF);
        offset += 4;
        for (int k=0; k<length_message; k++) {
            outbuffer[offset + k] = (byte)((this.message.getBytes())[k] & 0xFF);
        }
        offset += length_message;
        return offset;
    }

    public int deserialize(byte[] inbuffer, int start) {
        int offset = start;
        this.__id__  = (long)((inbuffer[offset + 0] & 0xFF) << (8 * 0));
        this.__id__ |= (long)((inbuffer[offset + 1] & 0xFF) << (8 * 1));
        this.__id__ |= (long)((inbuffer[offset + 2] & 0xFF) << (8 * 2));
        this.__id__ |= (long)((inbuffer[offset + 3] & 0xFF) << (8 * 3));
        offset += 4;
        this.result = (boolean)((inbuffer[offset] & 0xFF) != 0 ? true : false);
        offset += 1;
        int length_message = (int)((inbuffer[offset + 0] & 0xFF) << (8 * 0));
        length_message |= (int)((inbuffer[offset + 1] & 0xFF) << (8 * 1));
        length_message |= (int)((inbuffer[offset + 2] & 0xFF) << (8 * 2));
        length_message |= (int)((inbuffer[offset + 3] & 0xFF) << (8 * 3));
        offset += 4;
        byte[] bytes_message = new byte[length_message];
        for(int k= 0; k< length_message; k++){
            bytes_message[k] = (byte)(inbuffer[k + offset] & 0xFF);
        }
        this.message = new java.lang.String(bytes_message);
        offset += length_message;
        return offset;
    }

    public int serializedLength() {
        int length = 0;
        length += 1;
        int length_message = this.message.getBytes().length;
        length += 4;
        length += length_message;
        return length;
    }

    public java.lang.String getType() { return PUBANNOTATIONSDATA; }
    public java.lang.String getMD5(){ return "2c56a67188d7f4f2e190f74a00f52b49"; }
    public long getID() { return this.__id__; }
    public void setID(long id) { this.__id__ = id; }
}

}
