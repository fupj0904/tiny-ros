...
namespace gateway_msgs
{
  ...
  class GatewayInfo : public ros::Msg
  {
    ...
    virtual int serializedLength() const
    {
      ...
      length += 4;
      length += length_name;
      length += sizeof(this->connected);
      uint32_t length_ip = strlen(this->ip);
      length += 4;
      length += length_ip;
      length += sizeof(this->hub_names_length);
      for( uint32_t i = 0; i < hub_names_length; i++){
      uint32_t length_hub_namesi = strlen(this->hub_names[i]);
      length += 4;
      length += length_hub_namesi;
      }
      length += sizeof(this->hub_uris_length);
      for( uint32_t i = 0; i < hub_uris_length; i++){
      uint32_t length_hub_urisi = strlen(this->hub_uris[i]);
      length += 4;
      length += length_hub_urisi;
      }
      length += sizeof(this->firewall);
      length += sizeof(this->public_watchlist_length);
      for( uint32_t i = 0; i < public_watchlist_length; i++){
      length += this->public_watchlist[i].serializedLength();
      }
      length += sizeof(this->public_interface_length);
      for( uint32_t i = 0; i < public_interface_length; i++){
      length += this->public_interface[i].serializedLength();
      }
      length += sizeof(this->flipped_connections_length);
      for( uint32_t i = 0; i < flipped_connections_length; i++){
      length += this->flipped_connections[i].serializedLength();
      }
      length += sizeof(this->flip_watchlist_length);
      for( uint32_t i = 0; i < flip_watchlist_length; i++){
      length += this->flip_watchlist[i].serializedLength();
      }
      length += sizeof(this->flipped_in_connections_length);
      for( uint32_t i = 0; i < flipped_in_connections_length; i++){
      length += this->flipped_in_connections[i].serializedLength();
      }
      length += sizeof(this->pull_watchlist_length);
      for( uint32_t i = 0; i < pull_watchlist_length; i++){
      length += this->pull_watchlist[i].serializedLength();
      }
      length += sizeof(this->pulled_connections_length);
      for( uint32_t i = 0; i < pulled_connections_length; i++){
    ...
    const char * getMD5(){ return "4f7a5139edcaf513230dc74da5297dd0"; }
