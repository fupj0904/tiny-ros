...
static const char MAKENAVPLAN[] = "navfn/MakeNavPlan";
  ...
  class MakeNavPlanRequest : public ros::Msg
  {
    ...
    const char * getMD5(){ return "019fadf867addcf0070613002ff7e57b"; }
  ...
  class MakeNavPlanResponse : public ros::Msg
  {
    ...
    {
      int length = 0;
      length += sizeof(this->plan_found);
      uint32_t length_error_message = strlen(this->error_message);
      length += 4;
      length += length_error_message;
      length += sizeof(this->path_length);
      for( uint32_t i = 0; i < path_length; i++){
    ...
    const char * getType(){ return MAKENAVPLAN; }
    const char * getMD5(){ return "7f3e5cc0b0c6935d999b2b29304b1da9"; }
    const uint32_t getID(){ return this->__id__; }
    void setID(uint32_t id){ this->__id__ = id; }
