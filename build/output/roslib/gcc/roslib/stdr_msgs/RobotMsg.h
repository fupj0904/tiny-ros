...
namespace stdr_msgs
{
  ...
  class RobotMsg : public ros::Msg
  {
    ...
    virtual int serializedLength() const
    {
      ...
      length += sizeof(this->laserSensors_length);
      for( uint32_t i = 0; i < laserSensors_length; i++){
      length += this->laserSensors[i].serializedLength();
      }
      length += sizeof(this->sonarSensors_length);
      for( uint32_t i = 0; i < sonarSensors_length; i++){
      length += this->sonarSensors[i].serializedLength();
      }
      length += sizeof(this->rfidSensors_length);
      for( uint32_t i = 0; i < rfidSensors_length; i++){
      length += this->rfidSensors[i].serializedLength();
      }
      length += sizeof(this->co2Sensors_length);
      for( uint32_t i = 0; i < co2Sensors_length; i++){
      length += this->co2Sensors[i].serializedLength();
      }
      length += sizeof(this->soundSensors_length);
      for( uint32_t i = 0; i < soundSensors_length; i++){
      length += this->soundSensors[i].serializedLength();
      }
      length += sizeof(this->thermalSensors_length);
      for( uint32_t i = 0; i < thermalSensors_length; i++){
    ...
    const char * getMD5(){ return "8dee8be159ceb167303b8e278e43f295"; }
