...
namespace stdr_msgs
{
  ...
  class LaserSensorMsg : public ros::Msg
  {
    ...
    {
      int length = 0;
      length += sizeof(this->maxAngle);
      length += sizeof(this->minAngle);
      length += sizeof(this->maxRange);
      length += sizeof(this->minRange);
      length += sizeof(this->numRays);
      length += this->noise.serializedLength();
      length += sizeof(this->frequency);
      uint32_t length_frame_id = strlen(this->frame_id);
      length += 4;
    ...
    const char * getMD5(){ return "325111120547e280764866602306d7cd"; }
