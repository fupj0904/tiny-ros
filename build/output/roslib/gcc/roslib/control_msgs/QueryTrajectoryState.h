...
static const char QUERYTRAJECTORYSTATE[] = "control_msgs/QueryTrajectoryState";
  ...
  class QueryTrajectoryStateRequest : public ros::Msg
  {
    ...
    {
      int length = 0;
      length += sizeof(this->time.sec);
      length += sizeof(this->time.nsec);
      return length;
    }

    const char * getType(){ return QUERYTRAJECTORYSTATE; }
    const char * getMD5(){ return "f0a229bfb91bf05eb8a0dca02b9e560e"; }
  ...
  class QueryTrajectoryStateResponse : public ros::Msg
  {
    ...
    virtual int serializedLength() const
    {
      ...
      length += sizeof(this->acceleration_length);
      for( uint32_t i = 0; i < acceleration_length; i++){
      length += sizeof(this->acceleration[i]);
      }
      return length;
    }

    const char * getType(){ return QUERYTRAJECTORYSTATE; }
    const char * getMD5(){ return "c3c1aaf46de8952746d067ac8657a66e"; }
    const uint32_t getID(){ return this->__id__; }
    void setID(uint32_t id){ this->__id__ = id; }
