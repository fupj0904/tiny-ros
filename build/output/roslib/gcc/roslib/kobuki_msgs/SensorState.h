...
namespace kobuki_msgs
{
  ...
  class SensorState : public ros::Msg
  {
    ...
    virtual int serializedLength() const
    {
      ...
      length += sizeof(this->time_stamp);
      length += sizeof(this->bumper);
      length += sizeof(this->wheel_drop);
      length += sizeof(this->cliff);
      length += sizeof(this->left_encoder);
      length += sizeof(this->right_encoder);
      length += sizeof(this->left_pwm);
      length += sizeof(this->right_pwm);
      length += sizeof(this->buttons);
      length += sizeof(this->charger);
      length += sizeof(this->battery);
      length += sizeof(this->bottom_length);
      for( uint32_t i = 0; i < bottom_length; i++){
      length += sizeof(this->bottom[i]);
      }
      length += sizeof(this->current_length);
      for( uint32_t i = 0; i < current_length; i++){
      length += sizeof(this->current[i]);
      }
      length += sizeof(this->over_current);
      length += sizeof(this->digital_input);
      length += sizeof(this->analog_input_length);
      for( uint32_t i = 0; i < analog_input_length; i++){
      length += sizeof(this->analog_input[i]);
      }
      return length;
    }

    const char * getType(){ return "kobuki_msgs/SensorState"; }
    const char * getMD5(){ return "40f0ff84fa9c76e5f7b2f88121da23c6"; }
