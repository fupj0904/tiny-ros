...
namespace zeroconf_msgs
{
  ...
  class DiscoveredService : public ros::Msg
  {
    ...
    virtual int serializedLength() const
    {
      ...
      length += sizeof(this->ipv4_addresses_length);
      for( uint32_t i = 0; i < ipv4_addresses_length; i++){
      uint32_t length_ipv4_addressesi = strlen(this->ipv4_addresses[i]);
      length += 4;
      length += length_ipv4_addressesi;
      }
      length += sizeof(this->ipv6_addresses_length);
      for( uint32_t i = 0; i < ipv6_addresses_length; i++){
      uint32_t length_ipv6_addressesi = strlen(this->ipv6_addresses[i]);
      length += 4;
      length += length_ipv6_addressesi;
      }
      length += sizeof(this->port);
      length += sizeof(this->cookie);
      length += sizeof(this->is_local);
      length += sizeof(this->our_own);
      length += sizeof(this->wide_area);
      length += sizeof(this->multicast);
      length += sizeof(this->cached);
      return length;
    }

    const char * getType(){ return "zeroconf_msgs/DiscoveredService"; }
    const char * getMD5(){ return "0884be1c5130f30d1ba7d7cc396707e2"; }
